...
# ════════════════════════ VERCEL FLASK SERVER (API) ════════════════════════════════════════

def create_flask_app():
    """Create Flask app for Vercel serverless deployment."""
    try:
    ...
    def health():
        return jsonify({"status": "ok", "service": "Farm Advisory Voice Assistant"})
    ...
    def advisory():
        ...
        try:
            ...
            english = bhashini_translate_text(text, lang, 'en')
            
            # Get AI response
            answer_en = ask_groq(english)
            
            # Translate back to local language
            answer_local = bhashini_translate_text(answer_en, 'en', lang)
            
            # Generate TTS
            audio_local, _ = bhashini_translate_tts(answer_en, lang)
    ...
    def translate():
        ...
        try:
            result = bhashini_translate_text(text, src, tgt)
    ...
    def tts():
        ...
        try:
            audio_b64, audio_bytes = bhashini_translate_tts(text, lang)
            return jsonify({
                "audio": audio_b64,
                "sampling_rate": 8000
            })
...
if __name__ == "__main__":
    ...
    if args.mode == 'api':
        ...
        # Run as CLI (original terminal app)
        # Sanity checks
        try:
